(() => {
    ...
    const renderList = (items) => {
        ...
        for (const item of items) {
            ...
            const detail = el('p', '', (item.owner ? item.owner + ' · ' : '') + item.links_count + ' 个输入 · 更新于 ' + formatDate(item.updated_at, '未知') + ' · ' + state);
            ...
            const download = el('a', 'button', '下载 YAML');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch('/api/short-links', { method: 'POST', headers: Object.assign({ 'Content-Type': 'application/json' }, authHeaders()), body: JSON.stringify({ name: nameInput.value.trim(), target: 'clash', expires_in: Number(expiresInput.value), links }) });
            const data = await response.json().catch(() => ({}));
