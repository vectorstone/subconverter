...
1. **Protocol parsing and export** (`src/parser/subparser.cpp`, `src/parser/subparser.h`, `src/parser/config/proxy.h`, `src/generator/config/subexport.cpp`, `src/generator/config/singbox.cpp`, `src/generator/config/ruleconvert.cpp`): SS dialer/underlying-proxy parameters, VLESS + Reality parsing/export (including Surge/QuanX/Sing-box/mixed targets), TUIC parsing/export for Clash and Sing-box, IPv6 host normalization, and Clash dialer group generation with `DIRECT`. The sing-box skeleton target additionally emits the minimal group layout (`proxy` selector, `auto` urltest, `ChainProxyEntry`/`ChainProxyExit` when chained nodes exist, plus `GLOBAL` only on platforms with clash-mode rules), resolves the magic `x-sc-underlying-proxy=dialer` value to the `ChainProxyEntry` group (Clash parity), remaps preference group names onto the minimal groups, and converts bundled Surge lists to remote sing-geosite/sing-geoip rule-set references instead of inlining domains (`ruleconvert.cpp` mapping table).
